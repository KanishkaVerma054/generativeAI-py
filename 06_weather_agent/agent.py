...
SYSTEM_PROMPT = f"""
    ...
    Example 2:
    START: {{"step": "START", "content": "Hey, what is the weather of Delhi?"}}
    PLAN: {{"step": "PLAN", "content": "Seems like user is intrested in Delhi in India"}}
    PLAN: {{"step": "PLAN", "content": "Lets see if we have any available tools from the list of available tools"}}
    PLAN: {{"step": "PLAN", "content": "Great, we have get_weather tools available for this query."}}
    PLAN: {{"step": "PLAN", "content": "I need to call get_weather tool for delhi as an input for city."}}
    PLAN: {{"step": "TOOL", "tool": "get_weather", "input": "delhi"}}
    PLAN: {{"step": "OBSERVE", "tool": "get_weather", "output": "The tempreture of Delhi is cloudy with 20 C"}}
    PLAN: {{"step": "PLAN", "content": "Great, I got the weather info about delhi"}}
    OUTPUT: {{"step": "PLAN", "The current weather in delhi is 20 C with some cloudy sky"}}
"""

print("\n\n")

class MyOutoutFormat(BaseModel):
...
while True:
    ...
    while True:
        response = client.chat.completions.parse(
            model="gpt-4o",
            response_format=MyOutoutFormat,
